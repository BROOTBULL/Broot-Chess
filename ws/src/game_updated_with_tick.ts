...
export class Game {
  ...
  constructor(
    ...
    this.totalTimeMs = GAME_TIME_LIMITS[this.gameType as keyof typeof GAME_TIME_LIMITS] || (15 * 60 * 1000);
    this.startTicking();
  ...
  private getPlayer1TimeLeft() {
    return Math.max(0, this.totalTimeMs - this.getPlayer1TimeConsumed());
  }

  private getPlayer2TimeLeft() {
    return Math.max(0, this.totalTimeMs - this.getPlayer2TimeConsumed());
  }
  ...
  private startTicking() {
    ...
    this.tickInterval = setInterval(() => {
      ...
      // End game if any player is out of time
      if (whiteTimeLeft <= 0) {
